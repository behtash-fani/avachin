...
This directory is the repeatable P5-08 acceptance baseline. The manifest groups the regression tests that represent real Avachin acceptance paths: Unknown/local-first resolution, recording schema, online-to-offline learning, partial fingerprinting, bulk indexing and duplicates, AudD budget safety, temporary audio repair, runtime status, structured operations, and canonical entry points.
...
This lets Windows validation protect real source MP3 files while keeping the public corpus deterministic and license-safe.
